...
test('can view api keys', function(assert) {
	visit(Testing.SETTINGS_ROUTE)
		.then(function() {
			var $keySpan = $('.api-key-secret');
			$keySpan.find('a').click();
			assert.notEqual($.trim($keySpan.text()).indexOf(Balanced.NET.defaultApiKey), -1, 'Key does not match');
		});
});

test('can replace secret key', function(assert) {
	visit(Testing.SETTINGS_ROUTE)
		.then(function() {
			assert.equal(1, 1, 'boo');
		});
});
